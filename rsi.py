...
# Stochastic Buy/Sell Suggestions
st.markdown("### Stochastic Buy/Sell Suggestions")
stoch_col1, stoch_col2 = st.columns(2)

with stoch_col1:
    st.markdown("**Daily Stochastic Signals**")
    
    # Daily Stochastic signals
    daily_stoch_k = last_daily_stoch_k
    daily_stoch_d = last_daily_stoch_d
    
    stoch_daily_signals = []
    
    # SELL: Both %K and %D above 80
    if daily_stoch_k >= 80 and daily_stoch_d >= 80:
        stoch_daily_signals.append("🔴 SELL: Both %K and %D ≥ 80 (Overbought)")
    # BUY: Both %K and %D below 20
    elif daily_stoch_k <= 20 and daily_stoch_d <= 20:
        stoch_daily_signals.append("🟢 BUY: Both %K and %D ≤ 20 (Oversold)")
    # Bullish crossover: %K crosses above %D
    elif daily_stoch_k > daily_stoch_d and len(df_1d) > 1:
        prev_k = df_1d["STOCH_K"].iloc[-2] if not pd.isna(df_1d["STOCH_K"].iloc[-2]) else daily_stoch_k
        prev_d = df_1d["STOCH_D"].iloc[-2] if not pd.isna(df_1d["STOCH_D"].iloc[-2]) else daily_stoch_d
        if prev_k <= prev_d:  # Crossover occurred
            stoch_daily_signals.append("📈 Bullish Crossover: %K crossed above %D")
    # Bearish crossover: %K crosses below %D
    elif daily_stoch_k < daily_stoch_d and len(df_1d) > 1:
        prev_k = df_1d["STOCH_K"].iloc[-2] if not pd.isna(df_1d["STOCH_K"].iloc[-2]) else daily_stoch_k
        prev_d = df_1d["STOCH_D"].iloc[-2] if not pd.isna(df_1d["STOCH_D"].iloc[-2]) else daily_stoch_d
        if prev_k >= prev_d:  # Crossover occurred
            stoch_daily_signals.append("📉 Bearish Crossover: %K crossed below %D")
    
    if stoch_daily_signals:
        for signal in stoch_daily_signals:
            st.write(signal)
    else:
        st.info("🟡 No clear daily stochastic signals")
    
    st.caption(f"Daily Stoch: %K={daily_stoch_k:.1f}, %D={daily_stoch_d:.1f}")

with stoch_col2:
    st.markdown("**1-Minute Stochastic Signals**")
    
    if not df_1m.empty and not np.isnan(last_1m_stoch_k) and not np.isnan(last_1m_stoch_d):
        # 1-Minute Stochastic signals
        minute_stoch_k = last_1m_stoch_k
        minute_stoch_d = last_1m_stoch_d
        
        stoch_minute_signals = []
        
        # SELL: Both %K and %D above 80
        if minute_stoch_k >= 80 and minute_stoch_d >= 80:
            stoch_minute_signals.append("🔴 SELL: Both %K and %D ≥ 80 (Overbought)")
        # BUY: Both %K and %D below 20
        elif minute_stoch_k <= 20 and minute_stoch_d <= 20:
            stoch_minute_signals.append("🟢 BUY: Both %K and %D ≤ 20 (Oversold)")
        # Bullish crossover: %K crosses above %D
        elif minute_stoch_k > minute_stoch_d and len(df_1m) > 1:
            prev_k = df_1m["STOCH_K"].iloc[-2] if not pd.isna(df_1m["STOCH_K"].iloc[-2]) else minute_stoch_k
            prev_d = df_1m["STOCH_D"].iloc[-2] if not pd.isna(df_1m["STOCH_D"].iloc[-2]) else minute_stoch_d
            if prev_k <= prev_d:  # Crossover occurred
                stoch_minute_signals.append("📈 Bullish Crossover: %K crossed above %D")
        # Bearish crossover: %K crosses below %D
        elif minute_stoch_k < minute_stoch_d and len(df_1m) > 1:
            prev_k = df_1m["STOCH_K"].iloc[-2] if not pd.isna(df_1m["STOCH_K"].iloc[-2]) else minute_stoch_k
            prev_d = df_1m["STOCH_D"].iloc[-2] if not pd.isna(df_1m["STOCH_D"].iloc[-2]) else minute_stoch_d
            if prev_k >= prev_d:  # Crossover occurred
                stoch_minute_signals.append("📉 Bearish Crossover: %K crossed below %D")
        
        if stoch_minute_signals:
            for signal in stoch_minute_signals:
                st.write(signal)
        else:
            st.info("🟡 No clear 1-minute stochastic signals")
        
        st.caption(f"1-Min Stoch: %K={minute_stoch_k:.1f}, %D={minute_stoch_d:.1f}")
    else:
        st.info("No 1-minute stochastic data available")
